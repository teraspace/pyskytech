...
def query_thermo(request):
   ...
   # plates =  request.get('plates').replace('[','').replace(']','').replace('"', '\'')  
    ...
    sql = ' SELECT cathodic_historics.plate as station_name, driver as station_type, type_line, "CATHODIC_HISTORICS"."HICAFEEN", "CATHODIC_HISTORICS"."HICAVOSA", "CATHODIC_HISTORICS"."HICAVOSH", "CATHODIC_HISTORICS"."HICACOTU", "CATHODIC_HISTORICS"."HICAVOAC", "CATHODIC_HISTORICS"."HICACOAC", "CATHODIC_HISTORICS"."HICAESTA", cathodic_levels.canipome, cathodic_levels.canipoba, cathodic_levels.canivotu, cathodic_levels.canicotu, cathodic_levels.canivosh, (CASE WHEN length("CATHODIC_HISTORICS"."HICAPDON") > 2  THEN \'0\' ELSE "CATHODIC_HISTORICS"."HICAPDON" END) as hicapdon, (CASE WHEN length("CATHODIC_HISTORICS"."HICAPDOF") > 2  THEN \'0\' ELSE "CATHODIC_HISTORICS"."HICAPDON" END) as HICAPDOF FROM "CATHODIC_HISTORICS" INNER JOIN OWNER_PLATES ON OWNER_PLATES.plate = CATHODIC_HISTORICS.plate INNER JOIN CATHODIC_LEVELS ON CATHODIC_LEVELS.MOBILE_ID = CATHODIC_HISTORICS.MOBILE_ID WHERE CATHODIC_HISTORICS.mobile_id in ('+(mobile_ids)+') and "OWNER_PLATES"."OWNER_ID" = :owner_id AND (hicafeen between to_date(:fecha_inicial_reporte,\'dd/mm/yyyy hh24:mi:ss\') and to_date(:fecha_final_reporte,\'dd/mm/yyyy hh24:mi:ss\')) AND "OWNER_PLATES"."DRIVER" = :driver ORDER BY owner_plates.plate asc, CATHODIC_HISTORICS.HICAFEEN desc '
    print(sql)
    curs = conn.cursor()
    curs.prepare(sql)
