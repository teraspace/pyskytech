...
def history_cathodics_recti():
    ...
    data_report.replace('\n','', regex=True).replace('^<.*','0',regex=True).to_csv(full_path+'/history_cathodics_recti'+owner_id+'.csv', index=False)
...
def history_cathodics_thermo():
    ...
    start_time = time.time()
    data_report.replace('\n','', regex=True).replace('^<.*','0',regex=True).to_csv(full_path+'/history_cathodics_thermo'+owner_id+'.csv', index=False)
